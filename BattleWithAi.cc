...
coords BattleWithAi::getRandomLocation(char **aiBoard)
{
    coords randomLocation = {0, 0};
    randomLocation.columnNumber = getRandomRow();
    randomLocation.rowNumber = getRandomRow();
    if (aiBoard[randomLocation.rowNumber][randomLocation.columnNumber] == ' ')
    {
        return randomLocation;
    }
    return getRandomLocation(aiBoard);
}
...
bool BattleWithAi::shootAi(char **playerBoard, char **aiBoard, shipData **shipsArray, int **shipsQuantityArray)
{
    ...
    {
        shotLocation = getRandomLocation(aiBoard);
    }
    else
    {
        shotLocation = getPerfectShotLocation(aiBoard);
    }
